...
object Main {
  ...
  val basePath
    : String = System.getProperty("user.dir") + "/ldfi-akka/src/main/scala"
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (ctx.cpy) {
      val progDir = ctx.programsDir match {
        ...
        case Some(pd) =>
          sys.error("Dir: '" + pd.getCanonicalPath + "' does not exist")
        case None =>
          sys.error("No program directory was given. For usage, use --help")
      }
      ...
      val mainClass = ctx.mainClass match {
        ...
        case Some(mc) =>
          sys.error("Main Class: '" + mc.getCanonicalPath + "' does not exist")
      ...
      val verifyClass = ctx.verifyClass match {
        ...
        case Some(vc) =>
          sys.error(
            "Verify Class: '" + vc.getCanonicalPath + "' does not exist")
        case None =>
          sys.error("No verify class was given. For usage, use --help")
      }

      val verifyMethod = ctx.verifyMethod match {
        case null =>
          sys.error("No verify class method was given. For usage, use --help")
  ...
  def startEval(mainClass: File, verifyClass: File, verMeth: String): Unit = {
    ...
    val freePassMessages = try {
      ...
      case e: Exception =>
        sys.error(
          "Exception when reading from freePassMessages.txt. " +
  ...
  def reflectClass(cls: File, methodName: String): (Class[_], Method) = {
    try {
      ...
      val methodInst = methods.find(p => p.getName == methodName) match {
        ...
        case None =>
          sys.error(
            "Reflection error: could not find method: " + methodName + " in class: " + cls.getName)
      }
      (clazz, methodInst)
    } catch {
  ...
  def scalafixRewrite(): Unit = {
    ...
    val rewrite =
      "ldfi-akka/./scalafixCli --rules github:KTH/ldfi-akka/v1.0 " + basePath + " --sourceroot ldfi-akka" !
  ...
  def processOptions(args: List[String], ctx: Context): Context = args match {
    ...
    case "--copy" :: prDir :: rest =>
      processOptions(rest,
                     ctx.copy(cpy = true, programsDir = Some(new File(prDir))))
    ...
    case "-v" :: verClass :: verMeth :: rest =>
      processOptions(rest,
                     ctx.copy(verifyClass = Some(new File(verClass)),
                              verifyMethod = verMeth))

    case List() => ctx

    case unknown =>
      sys.error(
        "Option '" + unknown + "' was not recognized. For usage, use --help.")

  }

  def displayHelp(): Unit = {
    println(
      "Usage: ldfi-akka [options] -d <progdir> -m <main class> -v <verify class>")

    println(
      " --copy <myproject/>                             copy program in to ldfi-akka src dir")
    println(
      " --rewrite                                       rewrite program using scalafix")
    println(
      " -d <progDir>                                    target directory for scalafix rewrites")
    println(
      " -m <myproject/main>                             path to main file ")
    println(
      " -v <myproject/verifyclass.scala verifymethod>   path to file that returns boolean")
