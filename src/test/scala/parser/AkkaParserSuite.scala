...
class AkkaParserSuite extends FunSuite with Matchers {
  ...
  def testAkkaParser(): Unit = {
    ...
    logs.lift(1) match {
      case Some(input) =>
        ...
        val res: FormattedLogs = AkkaParser.parse(
          src,
          Set(MessageLit("A", "C", 1, "Broadcast(Some payload)")),
          List("Start"))
        test(
          "Testing AkkaParser, two messages sent with single injection at time 1") {
          assert(
            res == FormattedLogs(
              List(Row("A", "B", 1, "Broadcast(Some payload)"))))
        }
    ...
    logs.lift(1) match {
      case Some(input) =>
        val src = Source.fromString(input)
        val injections =
          List(MessageLit("A", "B", 1, "Broadcast(Some payload)"),
               MessageLit("B", "A", 2, "Broadcast(Some payload)"))
        val res: FormattedLogs =
          AkkaParser.parse(src, injections.toSet, List("Start"))
        test(
          "Testing AkkaParser, one messages sent with injection at first step & \"ghost\" time at step 2") {
          assert(
            res == FormattedLogs(
              List(Row("A", "B", 3, "Broadcast(Some payload)"))))
        }
      case None => println("testLogs are empty at position " + 1)
    }
    ...
    logs.lift(2) match {
      case Some(input) =>
        ...
        val res: FormattedLogs = AkkaParser.parse(src, Set.empty, List("Start"))
        test(
          "Testing AkkaParser, two same messages sent to same actor with no injections") {
    ...
    logs.lift(2) match {
      case Some(input) =>
        ...
        val res: FormattedLogs =
          AkkaParser.parse(src, injections.toSet, List("Start"))
        test(
          "Testing AkkaParser, two same messages sent with injection at first step & \"ghost\" time at step 2") {
          val rows = List(Row("A", "C", 1, "Broadcast(Some payload)"),
    ...
    logs.lift(3) match {
      case Some(input) =>
        ...
        val res: FormattedLogs = AkkaParser.parse(
          src,
          Set(MessageLit("A", "B", 1, "Broadcast(Some payload)"),
              MessageLit("B", "A", 2, "Broadcast(Some payload)")),
          List("Start"))
        test(
          "Testing AkkaParser, two diff messages sent with injection at first step & \"ghost\" time at step 2") {
          assert(
            res == FormattedLogs(
    ...
    logs.lift(4) match {
      case Some(input) =>
        ...
        val res: FormattedLogs = AkkaParser.parse(
          ...
          List("Start"))
        test(
          "Testing AkkaParser, different senders sent with injection at first step & \"ghost\" time at step 2") {
          assert(
            res == FormattedLogs(
              List(Row("A", "C", 1, "Broadcast(Some payload)"),
    ...
    logs.lift(5) match {
      case Some(input) =>
        ...
        val res: FormattedLogs = AkkaParser.parse(
          ...
          List("Start"))
        test(
          "Testing AkkaParser, different senders sent multiple messages " +
    ...
    logs.lift(7) match {
      case Some(input) =>
        val src = Source.fromString(input)
        val injections = List(
          MessageLit("B", "A", 3, "Broadcast(Some payload)"))
        val res: FormattedLogs =
          AkkaParser.parse(src, injections.toSet, List("Start"))
        test(
          "Testing AkkaParser, same sender sent multiple messages with injection at third step") {
          val rows =
            List(Row("A", "B", 1, "Broadcast(Some payload)"),
  ...
  def testmanageClock(): Unit = {
    ...
    test(
      "Assert that time does not tick when same injection sender and not injected") {
      assert(AkkaParser
        .manageClock("A", "B", "", "", 0, "Broadcast(Some payload)", inj1) == 1)
    }

    val inj2 = List(MessageLit("B", "A", 1, "Broadcast(Some payload)"))
    test("Assert that time ticks when not same injection sender") {
      assert(AkkaParser
        .manageClock("A", "B", "", "", 0, "Broadcast(Some payload)", inj2) == 2)
    }

    val inj3 = List(MessageLit("A", "B", 1, "Broadcast(Some payload)"))
    test("Assert that time ticks when same injection sender and injected") {
      assert(AkkaParser
        .manageClock("A", "B", "", "", 0, "Broadcast(Some payload)", inj3) == 2)
    }

    val inj4 = List(MessageLit("A", "B", 1, "Broadcast(Some payload)"),
                    MessageLit("B", "A", 2, "Broadcast(Some payload)"))
    test(
      "Assert that time ticks two steps when same injection and next senderinjector is different") {
      assert(AkkaParser
        .manageClock("A", "B", "", "", 0, "Broadcast(Some payload)", inj4) == 3)
    }
